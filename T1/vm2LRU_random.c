...
void newTLB(TLBlist* TLB, int page, int frame) {
        ...
        if (TLB->num <= TLB_Size) {//TLB未满
                ...
                delTLB(TLB);
                //加入node
                newTLB(TLB, page, frame);
        }
}

void newPage(int page, int rearFrameNum, char PhyMemory[][FRAME_SIZE], int pageCArecord[],int pageCAframe[]) {
    ...
    for(int i = 0; i <ADDRESSES_SIZE; i++){
        ...
        if (inTLB == 0) {//不在TLB中
                if (pageCArecord[page]) {//在内存并且不在TLB中
                        ...
                        printf("TLBHit is %d      ",TLBHit);
                        printf("page_default is %d      ",page_default);
                        newPage(page, rearFrameNum, PhyMemory, pageCArecord,pageCAframe);
                        newTLB(TLBC, page, frame);//注意page和TLB要同时更新
                        frame = rearFrameNum;
                        rearFrameNum++;
                }
